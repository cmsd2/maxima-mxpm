...
/// expanded. `doc_base_dir` is used to resolve relative image paths.
    ...
    for block in blocks {
        match block {
            parser::ParsedBlock::Section { title, body } => {
                ...
                let body_html = html::render_html(&body_md, doc_base_dir);
...
mod tests {
    ...
    fn parse_empty_document() {
        let idx = parse_markdown("", "pkg", "doc/pkg.md", Path::new(".")).unwrap();
    ...
    fn parse_function_symbol() {
        ...
        assert!(sym.body_html.contains("<em>name</em>"));
    }
    ...
    fn json_roundtrip() {
        let md = "## Intro\n\nHello.\n\n### Function: foo (x)\n\nDoes foo.\n\n```maxima\n(%i1) foo(1);\n(%o1) 42\n```\n\nSee also: `bar`.\n";
        let idx = parse_markdown(md, "test", "doc/test.md", Path::new(".")).unwrap();
    ...
    fn parse_testpkg_fixture() {
        ...
        let idx = parse_markdown(
            content,
            "testpkg",
            "doc/testpkg.md",
            Path::new("tests/fixtures/doc"),
        )
        .unwrap();
    ...
    fn parse_richpkg_fixture() {
        ...
        // HTML is rendered
        assert!(solve.body_html.contains("<strong>rich</strong>"));
        assert!(solve.body_html.contains("<em>multiple</em>"));
    }
}
